 ...
 * Kyqo Collection
 *
 * A fluent, chainable wrapper for arrays — inspired by Laravel Collections.
 * Provides a rich set of array manipulation methods.
 */
class Collection implements ArrayAccess, Countable, IteratorAggregate, JsonSerializable
{
    ...
    public function unique(string|callable $key = null): static
    {
        ...
        return $this->filter(function ($item) use ($key, &$seen) {
            $val = is_callable($key) ? $key($item) : (is_array($item) ? $item[$key] : $item->$key);
